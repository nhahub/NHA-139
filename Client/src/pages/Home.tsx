...
import { MapPin, DollarSign, Search, Star, Loader2,TrendingUp, HeadphonesIcon } from 'lucide-react';
  ...
  useEffect(() => {
    const fetchCities = async () => {
      try {
        ...
        const errorMessage = err.response?.data?.message || err.message || 'Failed to load cities';
            ...
            <div className="grid md:grid-cols-3 gap-4">
              ...
              <div className="relative">
                ...
                <div className="relative">
                  ...
                  >
                    <option className='dark:text-black' value="">All Cities</option>
                    {loadingCities && <option disabled>Loading cities...</option>}
                    {cities.map((city) => (
                      <option className='dark:text-black' key={city} value={city}>{city}</option>
                    ))}
                  </select>
            ...
            {(selectedCity || selectedPrice) && (
              <div className="mt-4 pt-4 border-t border-gray-200">
                <p className="text-sm text-gray-600">
                  ...
                  {selectedCity && <span className="font-semibold text-blue-600"> in {selectedCity}</span>}
                  {selectedCity && selectedPrice && ' with'}
                  {selectedPrice && <span className="font-semibold text-green-600"> {'$'.repeat(parseInt(selectedPrice))} price level</span>}
                </p>
              </div>
            )}
          </div>
  ...
  <div className="max-w-7xl mx-auto px-4">
    ...
    <div className="grid md:grid-cols-3 gap-12">
      <div className="group text-center transform hover:scale-105 transition-all duration-300 bg-gray-50 rounded-2xl p-8 shadow-lg hover:shadow-2xl border border-gray-100 dark:bg-[#1e293b]">
        ...
        <h3 className="text-2xl font-bold mb-3 text-gray-900  dark:text-white">Multiple Locations</h3>
        <p className="text-gray-600 text-lg leading-relaxed dark:text-gray-300">
          Explore amazing places across different cities and discover hidden gems wherever you go
        </p>
      </div>
      ...
      <div className="group text-center transform hover:scale-105 transition-all duration-300 bg-gray-50 rounded-2xl p-8 shadow-lg hover:shadow-2xl border border-gray-100 dark:bg-[#1e293b]">
        ...
        <h3 className="text-2xl font-bold mb-3 text-gray-900  dark:text-white">All Price Ranges</h3>
        <p className="text-gray-600 text-lg leading-relaxed dark:text-gray-300">
          Find options that perfectly fit your budget, from affordable to luxury experiences
        </p>
      </div>
      ...
      <div className="group text-center transform hover:scale-105 transition-all duration-300 bg-gray-50 rounded-2xl p-8 shadow-lg hover:shadow-2xl border border-gray-100 dark:bg-[#1e293b]">
        ...
        <h3 className="text-2xl font-bold mb-3 text-gray-900 dark:text-white">Top Rated</h3>
        <p className="text-gray-600 text-lg leading-relaxed dark:text-gray-300">
          Discover highly rated destinations trusted by thousands of satisfied visitors
        </p>
      </div>
    </div>
  </div>
